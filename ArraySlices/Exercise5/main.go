...
/*
TODO 1
Declares an array of 10 to 20 elements of type float64
use a const for the array size
Initialize a Slice from the Array, such that the Slice has the same length as the Array.
...
TODO 2
Using the slice, store random currency values using 'input.GetRandFloat()'.
TIP:
...
TODO 3
Use the slice to calculate total, max, min, avg
...
func main(){
   ...
   for i:=range slice{
	slice[i]=currency(input.GetRandFloat(10,20))

   }
   fmt.Printf("Slice is : %v , Type : %T",slice,slice)
