...
var noiseWords = map[string]bool{"on": true, "a": true, "the": true, "are": true, "in": true, "of": true}

/*Section 04 - Lab 01 : Word Count 1
TODO 1 - Count the occurrence of each word in a text file
Requirements
The filename is passed as an argument to the program
Use input.NewFileReader() to create an input.FileReader object.
The input.FileReader object has the method ReadLine().
You will also need strings.Split() to break lines into words.
TIP: I also used strings.TrimSpace() in my solution.*/

func main() {

	fileReader, err := input.NewFileReader(os.Args[1])
	if nil != err {
